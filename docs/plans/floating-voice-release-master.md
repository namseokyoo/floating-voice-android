...
**current_stage:** `V8-04 R4 SIGNED DEVICE APK PREPARATION`<br>
**status:** `v8_04_r4_automated_review_pass_signed_apk_pending` — 공식 서명 R3 APK를 A52s에서 검토한 결과 첫 문장 뒤 종료는 막았지만 350ms recognizer restart gap 동안 두 번째·세 번째 문장 앞부분이 일부 잘려 device FAIL했고 R3 APK는 `revoked/`로 격리했다. A52s가 Android 16임을 확인해 API 33+에서 한 physical recognizer/한 generation/audio lease를 유지하는 complete-silence segmented session으로 전환했다. 1,200ms 침묵마다 `onSegmentResults`를 누적하되 recognizer를 재생성하지 않으며, explicit Stop 뒤 `onEndOfSegmentedSession`에서만 review를 확정한다. segmented extra가 무시되어 일반 final이 오거나 Stop 전 session이 끝나면 gap restart로 숨기지 않고 visible draft를 보존한 채 CLIENT fail-closed한다. API 29~32는 기존 bounded chained fallback을 유지한다. controller 39 tests를 포함한 final clean 211 tasks, core 186/app debug 138/app release 134 tests, failures/errors 0, Debug/Release Lint 오류 0, segmented/STT Lint issue 0, localization 382/382를 통과했고 release DEX에 segmented intent extras와 두 callback 포함을 확인했다. 최초 독립 리뷰 High 1건(비정상 segmented end에서 latest partial 손실)을 RED→GREEN으로 닫았으며 bounded 후속 리뷰 PASS(Blocker 0/High 0, resolved 1/1)다. reviewed production controller SHA-256은 `500a40b0...e3e8b`다. 다음은 reviewed source commit/push와 공식 서명 R4 APK 생성·검증 후 형의 A52s 재검토다. STT Telegram 기본 대상/대상 변경/Android 공유 선택은 V8-05 계약이며 R4 device PASS 전 구현하지 않는다.<br>
...
| 4 | `0.8.0` | 시스템 STT 텍스트 공유·Android Sharesheet·로컬 OGG 출력 | [v0.8.0 STT·공유 계획](floating-voice-v0.8.0-stt-sharing.md) | **V8-04 R1 기능 PASS · R2 UI/연속 STT 수정 중** |
...
| 2026-08-14 A52s R1 | V8-04 기능 PASS + R2 수정 요구 | STT→review→Telegram 도착과 나머지 체크리스트 PASS. 투명·과대 review 화면을 작은 불투명 카드로 변경하고, 짧은 발화 종료 대신 사용자 중단까지 이어지는 chained dictation을 요구. STT 텍스트는 현재 Telegram 목적지를 기본으로 하되 다른 목적지 또는 Android 공유를 명시 선택하도록 V8-05 계약 확장. R1 APK 최종 승인 폐기, R2 재검토 필요 |
| 2026-08-14 15:53 KST | V8-04 R2 자동검증 PASS | 작은 불투명 중앙 카드(360dp max, 화면 높이 66% max, 전체 내부 스크롤)와 사용자 Stop까지 같은 generation/audio lease에서 fresh recognizer cycle을 잇는 dictation 구현. Stop 뒤 error/blank final에서도 현재 visible partial 보존, cancel/destroy 동기 callback 선무효화 보강. 최초 독립 리뷰 High 2 RED→GREEN, 후속 PASS(Blocker 0/High 0), clean 211 tasks, core 186/app debug 127/app release 123, Lint 오류 0, speech-review issue 0, localization 382/382. reviewed source commit/push와 공식 서명 R2 APK 준비 후 A52s 재검토 필요 |
