...
**current_stage:** `V8-05 OUTPUT ROUTE VERIFICATION`<br>
**status:** `v8_05_automated_gates_and_independent_review_pass_version_identity_commit_pending_r4_device_gate_pending` — V8-04 R4 segmented STT APK의 A52s 실기기 판정은 계속 대기한다. V8-05는 기본 Telegram 목적지·다른 검증 목적지 명시 선택·별도 Android 공유, content/output/destination identity+revision freeze, delivered 전 draft 유지, reject retry, handoff/attempt ID stale-callback 차단을 구현했다. Telegram 미설정 상태의 독립 STT·Android 공유 진입, 회전 중 destination/handoff/result 보존, 거절 feedback 유지, stale ordered-receipt UI 오염 차단까지 follow-up 수정했다. 최종 clean gate는 core 195/debug 149/release 145 tests(실패·오류·skip 0), localization 393/393, Lint Fatal/Error 0, Debug/Release assembly PASS다. 독립 follow-up review는 Blocker/High/Medium/Low 0으로 PASS했다. R4 `versionCode 19`보다 높은 V8-05 install identity `versionCode 20`/`versionName 0.8.0-v8-05`도 bounded review PASS했다.<br>
...
| 4 | `0.8.0` | 시스템 STT 텍스트 공유·Android Sharesheet·로컬 OGG 출력 | [v0.8.0 STT·공유 계획](floating-voice-v0.8.0-stt-sharing.md) | **V8-04 R4 APK 실기기 대기 · V8-05 TDD 진행** |
