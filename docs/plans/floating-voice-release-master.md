...
**current_stage:** `V8-08 INPUT/OUTPUT ROUTING`<br>
**status:** `v8_08_code26_rc2_gate_fixed_a52s_sign_gate_pending` — V8-06 R2 code 24는 형의 A52s 10-file save/reopen/play 사용자 직접 검증을 PASS했다. V8-07 code 25/name `0.8.0-v8-07-audio-sharesheet`는 scoped FileProvider, app-internal pending→retained 승격, active-file 삭제 차단, `ACTION_SEND audio/ogg` read grant/ClipData, no-recipient 진단, chooser-open/failed 전용 상태, no Telegram fallback, restart-safe 목록·확인 삭제를 구현했다. v8-08 code 26/name `0.8.0-v8-08-input-output-routing-rc2`에서 메뉴 단순화·입출력 라우팅을 구현했고, 텍스트 composer 진입 가드는 수정되어 로컬 text share 경로가 Telegram 미준비 상태에서도 열립니다. 자동검증은 test/lint/localization/clean build pass였고, 독립 review의 High 이슈(telegram 준비 미스)만 fix로 해소됨. Lint warning은 debug 49 / release 42이며 공식 서명 APK/RC 배포 hash는 미생성. A52s cross-UID URI readability/chooser 수신은 pending입니다.<br>
