...
**current_stage:** `V8-05 R3 STT SUPPORT-PREFLIGHT REMEDIATION`<br>
**status:** `v8_05_r3_code22_signed_candidate_ready_a52s_device_gate_pending` — code 21은 A52s에서 표준 recognizer 생성 후 `checkRecognitionSupport()`의 non-ready 결과를 절대 gate로 사용해 `startListening()`을 호출하지 않고 키보드 모드로 종료한 실기기 FAIL로 폐기했다. R3는 API 33+ 표준 recognizer 생성 성공 후 실제 듣기를 먼저 시작하고, READY/DOWNLOAD_REQUIRED/UNSUPPORTED/ERROR support 결과는 model download 없이 session 진단 정보로만 유지한다. 늦은 support 콜백도 active listening UI를 CHECKING으로 되돌리지 않으며 physical recognizer cycle 교체 뒤에도 logical session에 보존된다. terminal listener 예외가 나도 physical destroy와 audio ownership release를 마친 뒤 예외를 재전파한다. code 22/name `0.8.0-v8-05-r3`; targeted RED→GREEN과 final clean gate(core 195/debug 161/release 157 tests, 실패·오류·skip 0, localization 395/395, Lint Fatal/Error 0, 변경 Java warning/error 0, Debug/Release assembly)는 PASS다. 독립 follow-up review는 Blocker/High/Medium 0, Low 1로 PASS했고 남은 Low exact interleaving coverage도 전용 테스트 추가 후 final clean gate로 닫았다. app source commit `0c13c46fa68812999052edf2c1ea5acf685e9df6`에서 immutable fresh build 후 공식 인증서로 서명한 code 22 APK를 생성했고 SHA-256 `e67bfcbb7796903fdfdc4eaec47e02483da156a2c36888fc088c4ad81d3b67ed`, v2/v3·package/version·launcher·ABI·ZIP/ELF 16KB·credential scan을 통과했다. A52s 실기기 PASS는 아직 아니다.<br>
...
| 4 | `0.8.0` | 시스템 STT 텍스트 공유·Android Sharesheet·로컬 OGG 출력 | [v0.8.0 STT·공유 계획](floating-voice-v0.8.0-stt-sharing.md) | **V8-05 R3 자동·독립 review PASS · A52s 재검증 대기** |
