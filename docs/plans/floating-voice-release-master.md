...
**current_stage:** `V8-05 R2 CONTINUOUS STT + COMPACT REVIEW DEVICE REMEDIATION`<br>
**status:** `v8_05_r2_automated_and_independent_review_pass_commit_pending_a52s_device_gate_pending` — V8-05 code 20은 A52s Android 16에서 첫 문장 뒤 인식이 종료되고 review actions가 full-width 세로열로 길어지는 실기기 FAIL을 받았으며 폐기했다. R2는 API 33+에서 standard segmented recognizer를 우선하고, recognizer가 segmented mode를 무시해 regular final·premature end·NO_MATCH·TIMEOUT을 반환해도 visible partial을 누적한 뒤 같은 generation/ownership에서 즉시 새 cycle로 복구한다. Stop 재진입은 즉시 review로 확정하고 BUSY는 유효 텍스트 progress 사이에서 최대 3회로 제한한다. review UI는 목적지+변경 1행, Telegram/Android 2열, 다시 말하기/닫기 2열, 200% 글꼴에서만 responsive stack으로 재설계했다. code 21/name `0.8.0-v8-05-r2`, core 195/debug 156/release 152 tests(실패·오류·skip 0), localization 395/395, changed-file Lint warning/error 0, Debug/Release assembly와 최종 독립 review Blocker/High/Medium/Low 0을 통과했다. A52s에서 standard segmented 지원과 fallback restart gap은 새 서명 APK로 재검증해야 하며 아직 PASS가 아니다.<br>
...
| 4 | `0.8.0` | 시스템 STT 텍스트 공유·Android Sharesheet·로컬 OGG 출력 | [v0.8.0 STT·공유 계획](floating-voice-v0.8.0-stt-sharing.md) | **V8-05 R2 자동 gate PASS · A52s 재검증 대기** |
