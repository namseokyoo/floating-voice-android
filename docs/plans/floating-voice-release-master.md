...
**current_stage:** `V7-09 RC2 PREPARATION / RC1 REVOKED`<br>
**status:** `v7_09_rc2_preparation_rc1_revoked` — V7-08 자동 검증 후 만든 RC1/versionCode 18은 로그아웃 종료 전 늦은 catalog callback이 양수로 남아 있는 이전 account ID를 이용해 route를 재생성할 수 있다는 독립 리뷰 P1 때문에 실기기 승인 전에 폐기했다. RC2/versionCode 19는 route 초기화를 `AuthStage.READY && accountUserId > 0`으로 제한하고, 이전 TDLib client의 늦은 callback과 pending message-key 재할당을 차단하며, account close 시 pending dispatch를 UNKNOWN 보존하고 message mapping을 분리한다. 인증번호 단계에는 서버 timeout·next type을 따르는 재요청과 전화번호 수정 경로를 추가했다. 최신 clean core 165 + app Debug/Release 각 62, localization hard gate, Debug/Release Lint 오류 0, debug/release assembly와 bounded follow-up review P0–P2 0을 통과했다. commit-bound 공식 서명 RC2와 사용자 직접 A52s 검증이 다음 게이트다.<br>
